Feature: User who has enterered a valid code from Data Table IA Only adds another variation
    Description:
        Reference: DPL-3011
    
    Background: Given the user has completed the PL-3010 steps

    @DPL-3011
    Scenario: User directed to Enter Second Variation Code page
        ...
        And they will see a sub header 'What is the variation code or name?'
        And they will see a sub header 'Select a common variation'
        And they will see a sub header 'Choose from the full list of variations'
        And the user can see a text input box
        And they will see 5 radio button option(s)
        And they will see a 'View all variations' link
    ...
    @DPL-3011-5
    Scenario: User selects view all variations
        Given they will see a 'View all variations' link
        When the user has selected the link 'View all variations'
        Then they will be directed to the 'View all variations page' page

    @DPL-3011-6
    Scenario: User does not enter a valid code, select a radio button option, or link
        Given the user has been directed to the 'Enter Second Variation Code' page
        And they have not entered a valid 'Code'
        And they have not selected any 'radio button'
        And the user has not selected the link 'View all variations'
        When they select 'Continue'
