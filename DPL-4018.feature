Feature: User who has entered a valid code from Data Table IA Only  and a second variation code from Data Table IA Only checks, and is able to change answers
    ...
    Scenario Outline: User who has entered a valid code from Data Table IA Only and a second variation code from Data Table IA Only is directed to 'Check your answers' page 
        ...
        And a table header 'Supporting Documents'
        ...
        And a table with a row containing the list item 'Mock-Ups, if applicable. Only provide mock-ups if your variation is for the approval of mock-ups, e.g. C.II.6(b)' and the 'selection status' displayed and a 'Remove variation' link 
        And a table with a row with the list item 'Copy of the relevant page(s) from the Variation Guideline for the change(s) with the relevant boxes for conditions and documentation ticked' and the 'selection status' displayed and a change link
        And a table with a row with the list item 'Summary of the product characteristics (SPC), if applicable. If your variation affects the SPC, please provide proposed versions (in editable Word format) for each product included in the application' and the 'selection status' displayed and a change link
        And a table with a row with the list item 'Omitted Documents' and the 'omitted documents text' displayed and a change link
        And a table with a row with the list item Other Information' and the 'other information text' displayed and a change link
        And a section header 'Contact Details' with an 'Contact name' link
